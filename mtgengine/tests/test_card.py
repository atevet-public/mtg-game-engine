...
class TestCard:
    ...
    def test_card_initialization(self, name: str) -> None:
        ...
        card = Card(name, card_type="Instant", owner_index=1)
    ...
    def test_card_with_mana_cost(self) -> None:
        ...
        cost = ManaCost(red=1)
        card = Card("Lightning Bolt", card_type="Instant", mana_cost=cost, owner_index=1)
    ...
    def test_invalid_card_type_raises(self) -> None:
        ...
        with pytest.raises(ValueError):
            Card("Foo", card_type="InvalidType", owner_index=1)

    def test_numeric_fields_type_and_non_negative(self) -> None:
        """Numeric fields must be int or None and non-negative."""
        # Wrong type
        with pytest.raises(TypeError):
            Card("BadPower", card_type="Creature", power="five", owner_index=1)  # type: ignore[arg-type]

        # Negative value
        with pytest.raises(ValueError):
            Card("NegTough", card_type="Creature", toughness=-1, owner_index=1)

    def test_card_tap_sets_tapped_flag(self) -> None:
        """Calling tap should mark the card as tapped."""
        card = Card("Grizzly Bears", card_type="Creature", owner_index=1)
    ...
    def test_card_untap_clears_tapped_flag(self) -> None:
        ...
        card = Card("Llanowar Elves", card_type="Creature", owner_index=1)
        card.tap()
        assert card.tapped is True
    ...
    def test_card_rejects_invalid_owner_index(
        ...
        """Card should accept only int-like owner indexes."""
